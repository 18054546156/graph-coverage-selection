...
def assert_graphcov_source_unchanged():
    ...
    try:
        ...
        changed = subprocess.run(
            ["git", "-C", str(GRAPH_ROOT), "diff", "--name-only", EXPECTED_GRAPHCOV_COMMIT, "--", "graphcov"],
            check=True, capture_output=True, text=True,
        ).stdout.splitlines()
        if changed != ["graphcov/run/selection.py"]:
            raise RuntimeError(f"unexpected tracked GraphCov changes: {changed}")
        registry_text = (Path(GRAPH_ROOT) / "graphcov/run/selection.py").read_text(encoding="utf-8")
        if registry_text.count("from .d8_registry import register_d8_method") != 1:
            raise RuntimeError("D8 registration import is missing or duplicated")
        if registry_text.count("register_d8_method(register_method)") != 1:
            raise RuntimeError("D8 registration call is missing or duplicated")
...
def get_selection_data(name, train_224, train_28, info):
    needs_embedding = any(m in {"facility", "fps", "herding", "graph_a2", "d8_ot"} for m in METHODS)
    embeddings = None
    sources = {}
    if needs_embedding:
...
def save_selection_artifact(name, method, ratio, local_indices, source_train_indices, labels, budget, info, sources):
    ...
    method_sources = {}
    if method in {"facility", "fps", "herding", "graph_a2", "d8_ot"}:
...
def load_selection_artifact(name, method, ratio, source_train_indices, labels, info):
    ...
    if method == "d8_ot":
        expected_d8 = {"epsilon": 0.05, "outer_iterations": 8, "sinkhorn_iterations": 60}
        if config.get("d8_ot") != expected_d8:
            raise RuntimeError(f"D8 selector configuration mismatch at {directory}")
