...
EXPERIMENT_ROOT=/project/prj-sis01/xuxiaoyu/reliability_medmnistc_ab/table1_clean_only_5pct_20260922
LAUNCHER=$EXPERIMENT_ROOT/run_clean_only_5pct.slurm
QUEUE_FILE=${QUEUE_FILE:?QUEUE_FILE is required}
LOG_FILE=${LOG_FILE:-$EXPERIMENT_ROOT/logs/dispatcher_${USER}.log}
MAX_ACTIVE=${MAX_ACTIVE:-15}
JOB_PREFIX=table1-5pct-formal-${USER}
LOCK_FILE=${QUEUE_FILE}.lock

mkdir -p "$(dirname "$LOG_FILE")"
exec >>"$LOG_FILE" 2>&1
echo "5pct dispatcher started user=$USER queue=$QUEUE_FILE at=$(date -Is)"
...
while true; do
    ...
    if [[ -z "$line" ]]; then
        if (( active == 0 )); then
            echo "5pct dispatcher finished at=$(date -Is)"
            exit 0
        fi
        sleep 30
        continue
