...
def run_single(
    ...
    if sel_cache_key is not None and sel_cache_key in selection_cache:
        ...
        method_overrides = {
            ...
            'relation_mode': config.get('relation_mode', 'class_pair'),
            'cross_credit': config.get('cross_credit', 0.25),
            'relation_strength': config.get('relation_strength', 1.0),
            'pair_min_credit': config.get('pair_min_credit', 0.0),
            'pair_smoothing': config.get('pair_smoothing', 1.0),
        }
...
def run_experiment(config: Dict) -> str:
    ...
    def run_and_record(dataset, embedding, method, imp_method, ratio, trial):
        ...
        try:
            ...
            # Preserve exact selector output for paired reruns and equivalence audits.
            selected_array = np.ascontiguousarray(result['selected_indices'], dtype=np.int64)
            selected_sha256 = hashlib.sha256(selected_array.tobytes()).hexdigest()
            artifact_dir = run_dir / 'selection_artifacts'
            artifact_dir.mkdir(parents=True, exist_ok=True)
            ratio_tag = str(ratio).replace('.', 'p')
            embedding_tag = embedding or 'none'
            artifact_name = f"{dataset}_{embedding_tag}_{method}_r{ratio_tag}_s{seed}"
            artifact_path = artifact_dir / f"{artifact_name}.npy"
            np.save(artifact_path, selected_array)
